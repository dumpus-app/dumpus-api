...
def parse():
    ...
    with ZipFile(path) as zip:
        ...
        compute_parse_json_time = []
        for line in TextIOWrapper(zip.open(analytics_file_name)):

            compute_time_per_line_start = time.time()

            compute_parse_json_start = time.time()
            analytics_line_json = orjson.loads(line)
            compute_parse_json_end = time.time()
            compute_parse_json_time.append(compute_parse_json_end - compute_parse_json_start)
        ...
        print(f'Average parse json time: {sum(compute_parse_json_time) / len(compute_parse_json_time)}')
